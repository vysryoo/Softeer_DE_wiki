...
본 프로젝트의 궁극적인 목표는 웹 크롤링을 통해 수집한 현대자동차 관련 유튜브 댓글에 감정 분석 모델을 적용하여 소비자의 반응을 정량화하고, 특히 최근 가장 큰 주목을 받고 있는 IONIQ 9의 디자인과 관련된 핵심 여론을 분석하는 것입니다.

비록 전체적인 SUV와 Sedan의 경향성을 파악하는 것도 의미가 있으나, 실제 비즈니스 의사결정(마케팅 포인팅, 상품성 개선 등)에는 구체적인 차종 단위의 피드백이 훨씬 더 직접적으로 활용됩니다. 따라서 본 분석은 여러 차종의 전반적인 감정 반응을 비교한 뒤, 가장 긍정적인 지표를 보인 IONIQ 9을 중심으로 소비자들이 어떤 디자인 요소에 반응하는지 심층적으로 들여다보고자 설계되었습니다.
...
워드클라우드는 단순히 자주 등장하는 단어를 보여주는 데 그치지 않고, 소비자들이 어떤 요소를 중요하게 생각하는지 파악할 수 있게 해줍니다. 이를 통해 기업은 마케팅에서는 소비자가 긍정적으로 평가하는 요소를 강조하고, 제품 개발에서는 반복적으로 언급되는 불만 요소를 우선적으로 개선하는 데이터 기반 의사결정을 수행할 수 있습니다.
...
반대로 부정 비율이 높았던 AVANTE의 경우, 부정 댓글에서 자주 등장한 키워드를 통해 개선이 필요한 지점을 파악할 수 있다.
...
타겟을 너무 넓게 잡아서 의미 있는 인사이트를 도출하기 어려웠다
실제 사용자 입장에서 고려했어야 하는데, word cloud를 그리는 데에만 집중했다.
word cloud는 분석 수단이 되어야했는데 목적이 되어 버렸다.
word cloud에서 많이 나오는 키워드에 집중해서 주제를 좁히는 방향을 잡았으면 좋았을 것 같다.
디자인이나 홍보 관점에서는 일반 대중의 댓글이 의미가 있을 수 있지만, 성능이나 개선 포인트는 실 소유자의 댓글이 의미가 있기 때문에 이것을 구분할 수 있었으면 좋았을 것
데이터 전처리에서 컨텍스트를 더 반영하면 좋았을 것이다
Word cloud를 그렸을 때, 의미 있는 키워드가 포함되어 있었다면 해당 키워드를 활용하여 댓글을 샘플링하는 데에 활용할 수 있었을 것인데 관련 없는 키워드가 너무 많이 포함되어 있었다.
